...
func (c *ChecksCommand) Run(rawArgs []string) int {
	...
	{
		// We'll treat this warning as special because it should appear before
		// any other output from this command, whereas other diagnostics will
		// appear at the end of the output.
		var experimentDiags tfdiags.Diagnostics
		experimentDiags = experimentDiags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			`The "terraform checks" command is experimental`,
			"We're still exploring the possibilities with reporting check results, and so the presence and behavior of this command is subject to change even in Terraform CLI patch releases.",
		))
		view.Diagnostics(experimentDiags)
	}
